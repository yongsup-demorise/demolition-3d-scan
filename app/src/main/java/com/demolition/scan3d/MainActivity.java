...
public class MainActivity extends AppCompatActivity
  ...
  private final java.util.ArrayList<float[]> accumulatedPoints =
          new java.util.ArrayList<>();

  private static final int MAX_ACCUMULATED_POINTS = 200000;

  private final java.util.HashSet<String> pointKeys =
    ...
    if (session == null) {
      ...
      try {
        ...
        if (!session.isDepthModeSupported(
          ...
          status.setText(
                  "이 기기/카메라는 Raw Depth를 지원하지 않습니다"
          );
    ...
    try {
      ...
      switch (rotation) {
        ...
        case Surface.ROTATION_90:
          arRotation = Surface.ROTATION_90;
          break;

        case Surface.ROTATION_180:
          arRotation = Surface.ROTATION_180;
          break;

        case Surface.ROTATION_270:
          arRotation = Surface.ROTATION_270;
          break;

        case Surface.ROTATION_0:
        default:
          arRotation = Surface.ROTATION_0;
          break;
      }
      ...
      try (
        ...
        if (depth.getTimestamp()
          ...
          FloatBuffer accumulated =
                  accumulatePoints(xyz);
  ...
  private FloatBuffer makePoints(
          Camera cam,
          Image depth,
          Image confidence) {
    ...
    Image.Plane depthPlane =
            depth.getPlanes()[0];

    Image.Plane confPlane =
            confidence.getPlanes()[0];
    ...
    int dr =
            depthPlane.getRowStride();

    int dp =
            depthPlane.getPixelStride();

    int cr =
            confPlane.getRowStride();

    int cp =
            confPlane.getPixelStride();

    CameraIntrinsics intr =
            cam.getImageIntrinsics();

    float[] focal =
            intr.getFocalLength();

    float[] principal =
            intr.getPrincipalPoint();

    int[] dim =
            intr.getImageDimensions();
  ...
  private FloatBuffer accumulatePoints(FloatBuffer xyz) {
    ...
    while (xyz.remaining() >= 3
      ...
      int qx = Math.round(x * 100);
      int qy = Math.round(y * 100);
      int qz = Math.round(z * 100);
